...
## E0.3 Internal Contracts
### 이번 단계에서 추가한 계약

#### 1) Internal Contract File
- 파일: `packages/grid-core/src/core/grid-internal-contracts.ts`
- 역할:
  - `GridRendererPort`, `GridRendererState`, `GridVisibleRowRange`
  - `GridColumnStatePort`
  - `GridDerivedViewRowModelPort`
  - `GridExportDataPort`
  - `GridColumnMutationPort`, `GridDerivedViewControllerPort`, `GridAuditLogPort`

#### 2) 적용 위치
- `packages/grid-core/src/core/grid.ts`
  - `Grid`는 concrete `DomRenderer` 대신 `GridRendererPort`에 의존한다.
- `packages/grid-core/src/render/dom-renderer.ts`
  - `DomRenderer`는 `GridRendererPort`를 구현한다.
- `packages/grid-core/src/core/grid-state-service.ts`
  - column state 조작은 `GridColumnStatePort`를 사용한다.
- `packages/grid-core/src/core/grid-data-pipeline-service.ts`
  - derived view row model 조작은 `GridDerivedViewRowModelPort`를 사용한다.
- `packages/grid-core/src/core/grid-remote-query-service.ts`
  - remote provider sync는 `GridDerivedViewRowModelPort`를 사용한다.
- `packages/grid-core/src/core/grid-export-service.ts`
  - export 입력은 `GridExportDataPort` 기반으로 고정한다.
- `packages/grid-core/src/core/grid-command-event-service.ts`
  - column mutation / derived view / audit log 의존성은 contract port로 묶는다.

### ADR
- DOM-only 유지: `docs/adr-dom-only-core.md`
- pooling invariants: `docs/adr-pooling-invariants.md`
- virtualization invariants: `docs/adr-virtualization-invariants.md`

### 현재 효과
- `Grid`, renderer, service가 concrete class보다 contract type을 기준으로 연결된다.
- service가 `Grid` private field 집합에 직접 기대지 않고, 필요한 capability만 port로 받는다.
- 이후 `E1 Worker Runtime`, `E2 server-side row model` 작업에서도 영향 범위를 port 단위로 제한할 수 있다.

### 다음 단계
- `E1`에 들어가기 전에 필요하면 `grid.ts`의 option normalization/derived view orchestration도 별도 contract boundary로 더 분리할 수 있다.
- 현재 체크리스트 기준 다음 우선순위는 `Phase E1 — Actual Worker Runtime`이다.
