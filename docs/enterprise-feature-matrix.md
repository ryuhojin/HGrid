...
| Remote Data Provider | 부분 완료 | block cache, prefetch, LRU, query model 제공. E2.1 기준으로 `serverSide` envelope(schema version, route, partial/full store strategy)와 row metadata contract(child count, aggregate values)까지 고정 | AG Grid SSRM 수준의 store scheduling, remote group/pivot/tree rendering, cache invalidation model은 아직 아님 | [remote-data-provider.ts](../packages/grid-core/src/data/remote-data-provider.ts), [remote-data-provider-phase8.md](./remote-data-provider-phase8.md), [server-side-row-model-phase-e2.md](./server-side-row-model-phase-e2.md) |
